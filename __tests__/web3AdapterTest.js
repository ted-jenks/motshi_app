...
describe("web3Adapter tests", function() {
  ...
  const issuerNodeKey = "199f5867803e6dac9d8ccc0118761b4140aa657d448459cc920aa198925e4e1b"; // Do not use this key in production, it is strictly for the testNet
  const account = {
    "address": issuerNode, "privateKey": issuerNodeKey,
  };
  ...
  it("can unlock an account", async () => {
    await web3Adapter.unlockAccount()
      .then(r => {
        assert(r);
      });
  });
  ...
  it("can modify data on smart contract", async () => {
    const contract = web3Adapter.getContract(CERTIFICATION_SERVICE_ABI, contractAddress, issuerNode); // Use issue node as node knows private key
  ...
  it("can search by hash on smart contract", async () => {
    const result = await web3Adapter.searchByHash("ff4007ffffffffc0fe0003ffffffff80f80000fffffffe00f000007ffffffe00ff4007ffffffffc0fe0003ffffffff80f80000fffffffe00f000007ffffffe00");
