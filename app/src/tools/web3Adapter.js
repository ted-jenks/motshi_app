...
class Web3Adapter {
  constructor(web3, contractAddress, account) {
    ...
    this.contract = new this.web3.eth.Contract(CERTIFICATION_SERVICE_ABI, contractAddress, {
  ...
  async unlockAccount() {
    try {
      await this.web3.eth.personal.unlockAccount(this.account.address, this.account.privateKey, 1000);
      return true;
    } catch {
      return false;
    }
  ...
  async searchByHash(hash) {
    await this.unlockAccount();
    try {
      let hash_2 = await this.contract.methods.data_hash("0x".concat(hash.substring(0, 64))).call();
  ...
  async issueCertificate(address,
                         data_hash_to_submit,
                         // salt_hash_to_submit,
